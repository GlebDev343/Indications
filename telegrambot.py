...
def check_pesonal_account(message):
    try:
        ...
        bot.send_message(message.chat.id, "A confirmation code has been sent to your email, please enter the message in the format (indication code)")
    ...
    try:
        ...
        for i in range(len(message.text)):
            if message.text[i] != " ":
                ...
                break
        if PersonalAccount.objects.get(account_number=account_number, verification_code=verification_code).verification_code == verification_code:        
